resource "aws_eks_cluster" "eks" {
  ...
  vpc_config {
    subnet_ids = [aws_subnet.pri_subnet[0].id,aws_subnet.pri_subnet[1].id,aws_subnet.pri_subnet[2].id]
...
resource "aws_iam_openid_connect_provider" "eks-oidc" {
  ...
  thumbprint_list = [data.tls_certificate.eks-cft.certificates[0].sha1_fingerprint]
  url = data.tls_certificate.eks-cft.url
}
